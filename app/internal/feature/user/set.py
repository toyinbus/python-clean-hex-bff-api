...
class UserFeature:
    ...
    def register_http(self, api: APIRouter) -> None:
        api.include_router(register_routes(self._handler, self._auth))


class UserContainer(containers.DeclarativeContainer):
    config = providers.Configuration()
    database = providers.Dependency()  # shared pool holder (from the app container)
    auth_guard = providers.Dependency()  # shared JWT guard (from AuthContainer)
    ...
    feature = providers.Singleton(UserFeature, handler=handler, auth=auth_guard)
